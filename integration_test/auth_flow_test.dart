...
void main() {
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('auth_flow_001: New donor signs up and gets routed to profile setup',
    ...
    final doc =
        await mockFirestore.collection('users').doc('donor_new_001').get();
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('auth_flow_002: Admin logs in and gets routed to AdminDashboard',
      (WidgetTester tester) async {
    // Arrange: admin user
    final mockUser = MockUser(
      uid: 'admin_001',
    ...
    final doc =
        await mockFirestore.collection('users').doc('admin_001').get();
    final roles = List<String>.from(doc.data()!['roles']);
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('auth_flow_003: Purchaser signs up with correct Firestore document structure',
      (WidgetTester tester) async {
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('auth_flow_004: Sign out clears auth state and redirects to login',
      (WidgetTester tester) async {
    final mockUser = MockUser(
      uid: 'user_signout',
  ...
  testWidgets('auth_flow_005: Admin updates user role in Firestore',
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: Center(
            child: Text('New role: ${roles.first}'),
          ),
        ),
      ),
    );
