...
void main() {
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('delivery_flow_001: Admin creates delivery assignment for a family',
    ...
    final doc =
        await mockFirestore.collection('delivery_assignments').doc('del_001').get();
          ...
          body: Center(
            child: Column(
              ...
              children: [
                ...
                Text('Family: ${assignment.familyArea}, ${assignment.familyCity}'),
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('delivery_flow_002: Distributor marks pickup — status becomes picked_up',
      (WidgetTester tester) async {
    final now = Timestamp.now();

    // Seed a not_started assignment
    await mockFirestore.collection('delivery_assignments').doc('del_pickup').set({
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('delivery_flow_003: Full delivery lifecycle from assignment to delivered',
      (WidgetTester tester) async {
    final now = Timestamp.now();

    await mockFirestore.collection('delivery_assignments').doc('del_full').set({
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('delivery_flow_004: Failed delivery is reassigned to new distributor',
      (WidgetTester tester) async {
    final now = Timestamp.now();

    await mockFirestore.collection('delivery_assignments').doc('del_fail').set({
          ...
          body: Center(
            child: Column(
              ...
              children: [
                ...
                Text('Failure Reason: ${assignment.failureReason!.toFirestore()}'),
              ],
            ),
          ),
        ),
  ...
  // ─────────────────────────────────────────────────────────────────
  testWidgets('delivery_flow_005: Admin verifies delivered proof — status becomes admin_verified',
      (WidgetTester tester) async {
    final now = Timestamp.now();

    // Seed a delivered assignment
    await mockFirestore.collection('delivery_assignments').doc('del_verify').set({
